...
#read in the data
#cov_full<-as.matrix(read.table("/projects/seedpod/output/pcangsd_SC/spal/spal_full_filt.cov"))
cov_full<-as.matrix(read.table("/projects/seedpod/output/pcangsd_SC/spal/filt_full_list.beagle.gz.cov"))
...
plot(e_full$vectors[,1:2], col=sites_full$colors, pch=sites_full$pchers, xlab="PC1 (5.1%)", ylab="PC2 (4.4%)")

###########
#for source sites
cov_s<-as.matrix(read.table("/projects/seedpod/output/pcangsd_SC/spal/filt_source_list.beagle.gz.cov"))
e_s<-eigen(cov_s)
...
plot(e_s$vectors[,1:2], col=sites_s$colors, pch=sites_s$pchers)

#for greenhouse

cov_g<-as.matrix(read.table("/projects/seedpod/output/pcangsd_SC/spal/filt_grh_list.beagle.gz.cov"))
e_g<-eigen(cov_g)
...
plot(e_g$vectors[,1:2], col=sites_g$colors, pch=sites_g$pchers)


#for nursery

cov_n<-as.matrix(read.table("/projects/seedpod/output/pcangsd_SC/spal/filt_nur_list.beagle.gz.cov"))
e_n<-eigen(cov_n)
...
cov_g_s<-as.matrix(read.table("/projects/seedpod/output/pcangsd_SC/spal/filt_source_grh_list.beagle.gz.cov"))
e_g_s<-eigen(cov_g_s)
...
sites_g_s$colors<-ifelse(sites_g_s$V3 == "GRH" & sites_g_s$V4 == "PIS", "gold2", 
                         ifelse(sites_g_s$V3 == "GRH" & sites_g_s$V4 == "RUM", "gold3", 
                                ifelse(sites_g_s$V3 == "GRH" & sites_g_s$V4 == "BEL", "gold", 
                                       ifelse(sites_g_s$V3 == "BEL", "paleturquoise", 
                                              ifelse(sites_g_s$V3 == "ROW" , "paleturquoise2",
                                                     ifelse(sites_g_s$V3 == "RUM" , "paleturquoise3", "na"))))))
...
plot(e_g_s$vectors[,1:2], col=sites_g_s$colors, pch=sites_g_s$pchers)
...
par(mfrow=c(2,3))
plot(e_full$vectors[,1:2], col=sites_full$colors, pch=19, xlab="PC1 (4.5%)", ylab="PC2 (3.4%)", main="All samples")
plot(e_n$vectors[,1:2], col=sites_n$colors, pch=19, xlab="PC1 (11.5%)", ylab="PC2 (3.0%)", main="Nursery samples")
plot(e_s$vectors[,1:2], col=sites_s$colors, pch=19, xlab="PC1 (4.4%)", ylab="PC2 (4.3%)", main="Source samples")
plot(e_g$vectors[,1:2], col=sites_g$colors, pch=19, xlab="PC1 (6.5%)", ylab="PC2 (4.4%)", main="Greenhouse samples")
plot(e_g_s$vectors[,1:2], col=sites_g_s$colors, pch=19, xlab="PC1 (5.2%)", ylab="PC2 (3.1%)", main="Greenhouse + Source samples")
...
pch_full<-merge(sites_full, the_def,sort=F)
pch_n<-merge(sites_n,the_def, sort=F)
pch_s<-merge(sites_s, the_def, sort=F)
